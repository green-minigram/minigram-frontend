...
void _goToPasswordStep() {
  final ctx = navigatorKey.currentContext;
  if (ctx == null) return;
  Navigator.pushNamed(ctx, MRoute.joinPassword);
}
...
bool handleLocalUsernameValidation(
  JoinModel state,
  void Function(String err) syncUsernameError,
) {
  final usernameErr = validateUsername(state.username);
  if (usernameErr.isEmpty) return false; // 통과 → 다음 가드로

  // 오류 메시지 표출 및 상태 동기화
  _showSnack(usernameErr);
  syncUsernameError(usernameErr);
  return true; // 여기서 종료
}
...
bool handleUsernameCheckingInProgress(JoinModel state) {
  if (!state.isUsernameChecking) return false;
  _showSnack("아이디 중복 확인 중입니다. 잠시만 기다려주세요.");
  return true;
}
...
bool handleUsernameAvailabilityFailure(
  JoinModel state,
  void Function(String err) syncUsernameError,
) {
  if (state.isUsernameAvailable == true) return false; // 사용 가능 → 통과

  final msg = state.isUsernameAvailable == false ? "이미 사용 중인 아이디입니다." : "아이디 중복 확인이 필요합니다.";
  _showSnack(msg);

  // 중복 확정(false)일 때는 상태에도 에러 반영(UX 일관성)
  if (state.isUsernameAvailable == false) {
    syncUsernameError("이미 사용 중인 아이디입니다.");
  }
  return true;
}
...
void goToNextStep() => _goToPasswordStep();

// 공개 API: 이메일 확인 플로우
// - 위 1~3 가드를 순서대로 실행하고, 모두 통과하면 4로 이동.
// - VM에서는 이 함수만 호출하면 됨.
// 반환: 성공적으로 다음 화면으로 이동했으면 true, 중간에 막혔으면 false
bool confirmUsernameFlow({
  required JoinModel state,
  required void Function(String err) syncUsernameError,
}) {
  if (handleLocalUsernameValidation(state, syncUsernameError)) return false;
  if (handleUsernameCheckingInProgress(state)) return false;
  if (handleUsernameAvailabilityFailure(state, syncUsernameError)) return false;
  goToNextStep();
  return true;
}
